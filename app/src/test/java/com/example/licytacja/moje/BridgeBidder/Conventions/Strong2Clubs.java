...
public class Strong2Clubs extends Bidder {
    ...
    private static PositionCalls respond(PositionState ps) {
        ...
        if (ps.getRHO().isPassed()) {
            ...
            choices.addRules(shows(Bid._2H, points(POSITIVE_RESPONSE), shape(5, 11), GOOD_PLUS_SUIT));
            choices.addRules(shows(Bid._2S, points(POSITIVE_RESPONSE), shape(5, 11), GOOD_PLUS_SUIT));
            choices.addRules(shows(Bid._2NT, points(POSITIVE_RESPONSE), BALANCED));
            choices.addRules(shows(Bid._3C, points(POSITIVE_RESPONSE), shape(5, 11), GOOD_PLUS_SUIT));
            choices.addRules(shows(Bid._3D, points(POSITIVE_RESPONSE), shape(5, 11), GOOD_PLUS_SUIT));

            choices.addRules(properties(Bid._2D, Strong2Clubs::openerRebidWaiting, true));
            choices.addRules(shows(Bid._2D, points(WAITING)));
        }
        return choices;
    }
    ...
    private static PositionCalls responder2ndBid(PositionState ps) {
        ...
        choices.addRules(shows(Bid._3C, shape(5, 11)));
        choices.addRules(shows(Bid._3D, shape(5, 11)));
        choices.addRules(shows(Bid._3H, shape(5, 11)));
